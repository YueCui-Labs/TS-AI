...
def dice_parc(label_0, label_turb, label_num):
    ...
    for ll in range(label_num):
        dice = 2 * ((label_0 == ll) & (label_turb == ll)).sum() / ( (label_0 == ll).sum() + (label_turb == ll).sum() + 1e-5)
...
prename = args.syn_prename
print(prename)

model_dir = '{}_fold{}'.format(prename, args.fold)
print(model_dir)
...
if __name__ == "__main__":
    ...
    test_dataset = unet_syn_indiv_dataset(
        test_sub_list_file,
        modals,
        out_modal,
        feat_dir,
    ...
    model = Unet_2branch(modals, out_modal, in_chs, label_num, istrain=False)
    model.to(device)
    ...
    for feats, masks, label, sub in tqdm(test_dataloader):
        if not os.path.exists("{}.32k_fs_LR.func.gii".format(os.path.join(feat_dir, sub[0], prename+'_task_contrast24.'+hemi))) or \
            not os.path.exists('{}.32k_fs_LR.label.gii'.format(os.path.join(indiv_label_dir, sub[0], prename))):
            model.eval()
            B = label.shape[0]
            ...
            syn, idv, recon, _, _ = model(feats)

            save_recon(label_40k, syn[0,...].T.detach().cpu().numpy(), label_32k_file,
                       prename=os.path.join(feat_dir, sub[0], prename+'_task_contrast24.'+hemi)
                       )
