...
from model import Unet_2branch
from config import parse_args
from dataset import unet_syn_indiv_dataset
from loss import syn_indiv_loss
...
def train_one_step(feats, out_feat, label):
    """
    ...
    # with autocast():
    syn, pred_parc, feat_parc_avg = model(feats)
    loss_tot, loss_ce, loss_feat, loss_syn = criterion(pred_parc, label, feats, feat_parc_avg, out_feat, syn)
    ...
    metrics = (*evaluate_indiv(pred_parc, label, feats, feat_parc_avg),
               evaluate_syn(out_feat, syn))

    return metrics, (loss_tot.item(), loss_ce.item(), loss_feat.item(), loss_syn.item())

def evaluate_indiv(logits, labels, recon, feats):
    """
    :param logits: (B,K,V)
    :param labels: (B,V)
    ...
    for i, (rec, fea) in enumerate(zip(recon, feats)):
        ...
        pearsonr += weight_recon[i] * corrcoef(rec.float().reshape(-1, logits.shape[2]), fea.float().reshape(-1, logits.shape[2]))
    return (ov.item(), nparc, pearsonr)


def evaluate_syn(recon, feats):
    """
    :param label: (B,V)
...
def val_during_training(dataloader):
    ...
    for feats, out_feat, label, _ in dataloader:
        ...
        val_losses.update(
            [loss_tot.item(), loss_ce.item(), loss_feat.item(), loss_syn.item()],  n=B)
...
lam_feat = int(args.lamb)
lam_syn = int(args.lamb_syn)

weight_recon = [1/len(modals) for _ in modals]

base_dir = os.path.join(
    save_dir, '{}_fold{}'.format(args.syn_prename, args.fold)
)
...
if __name__ == '__main__':
    ...
    train_dataset = unet_syn_indiv_dataset(
        train_sub_list_file,
        args.modals,
        out_modal,
        feat_dir,
    ...
    val_dataset = unet_syn_indiv_dataset(
        vali_sub_list_file,
        args.modals,
        out_modal,
        feat_dir,
    ...
    model = Unet_2branch(modals, out_modal, in_chs, label_num)
    ...
    criterion = syn_indiv_loss(lam_feat, lam_syn)
    ...
    for epoch in range(start_epoch, epoch_num):
        ...
        for i, (feats, out_feat, label, _) in enumerate(tqdm(train_dataloader)):
            B = label.shape[0]
            evals, losses = train_one_step(feats, out_feat, label)
        ...
        print('<<<Train>>>  Loss: {loss:.4f}, atlas: {atlas:.4f}, feat: {feat:.4f}, syn: {syn:.4f} || Overlap: {ov:.4f}, Parc Num.: {pn: .4f} | infeatR: {infeatR:.4f}, outfeatR: {outfeatR:.4f}'.format(
            loss=train_losses_avg[0],
            atlas=train_losses_avg[1],
            feat=train_losses_avg[2],
            syn=train_losses_avg[3],

            ov=train_metrics_avg[0],
            pn=train_metrics_avg[1],
            infeatR=train_metrics_avg[2],
            outfeatR=train_metrics_avg[3]
        ))

        # validation
        val_losses_avg, val_metrics_avg = val_during_training(val_dataloader)
        print('<Validation>  Loss: {loss:.4f}, atlas: {atlas:.4f}, feat: {feat:.4f}, syn: {syn:.4f} || Overlap: {ov:.4f}, Parc Num.: {pn: .4f} | infeatR: {infeatR:.4f}, outfeatR: {outfeatR:.4f}'.format(
            loss=val_losses_avg[0],
            atlas=val_losses_avg[1],
            feat=val_losses_avg[2],
            syn=val_losses_avg[3],

            ov=val_metrics_avg[0],
            pn=val_metrics_avg[1],
            infeatR=val_metrics_avg[2],
            outfeatR=val_metrics_avg[3]
        ))
